...
const keywordCompletions: readonly KeywordCompletion[] = [
  ...
  { label: "materialize", detail: "Materialize a query", contexts: ["topLevel"], snippet: "materialize ${1:name} as\n  show ${2:metric} by ${3:dimension}",
    doc: "Emits `CREATE MATERIALIZED VIEW <name> AS <query>` for a saved query, and registers it as a pre-aggregate the planner may answer later queries from.\n\n```sem\nmaterialize daily_revenue as\n  show revenue, orders by region, ordered_at.day\n```\nA query is routed to it only when every metric it asks for can be recovered exactly: additive metrics are re-aggregated (`sum`, `min`, `max`), coarser time grains are re-truncated, and anything that would be mis-rolled — a semi-additive balance, a ratio, a fan-out grain — falls back to the fact table instead." },
