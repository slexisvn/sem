...
const keywordCompletions: readonly KeywordCompletion[] = [
  ...
  { label: "materialize", detail: "Materialize a query", contexts: ["topLevel"], snippet: "materialize ${1:name} as\n  show ${2:metric} by ${3:dimension}",
    doc: "Emits `CREATE MATERIALIZED VIEW <name> AS <query>` for a saved query, and nothing else — queries keep reading the fact tables.\n\n```sem\nmaterialize monthly_revenue as\n  show revenue, revenue.mom by ordered_at.month\n```\nUse `rollup` instead when the view should also answer queries." },
  { label: "rollup", detail: "Materialize a query and serve queries from it", contexts: ["topLevel"], snippet: "rollup ${1:name} as\n  show ${2:metric} by ${3:dimension}",
    doc: "Builds the view exactly as `materialize` does, and registers it as a pre-aggregate the planner may answer later queries from.\n\n```sem\nrollup daily_revenue as\n  show revenue, orders by region, ordered_at.day\n```\nA query is routed to it only when every metric it asks for can be recovered exactly: additive metrics are re-aggregated (`sum`, `min`, `max`), coarser time grains are re-truncated, and anything that would be mis-rolled — a semi-additive balance, a ratio, a fan-out grain — falls back to the fact table instead. `routedTo` on the compile result names the rollup that answered, or is undefined when the fact table did.\n\nThe body must be a plain aggregate. A transform, `having`, or `top` has already thrown away the rows a later query would need, so declaring one is an error — use `materialize` for those." },
  { label: "show", detail: "Start a query", contexts: ["topLevel", "query"], snippet: "show ${1:metric} by ${2:dimension}",
    doc: "The query form: pick metrics, group them, filter, sort.\n\n```sem\nshow revenue, aov by region where region = 'VN' order by revenue desc top 5\n```\nMetrics can be combined with `+ - * /` and compared against their own subtotals; an inline expression needs a name.\n\n```sem\nshow revenue / revenue.of(region) as share_of_region by region, status\n```\nQueries live outside `.sem` model files; run one with **Sem: Compile Query to SQL**." },
  ...
  { label: "retention", detail: "Cohort-by-period retention matrix", contexts: ["topLevel", "query"], snippet: "retention ${1:Model} by ${2:entity_id} over ${3:occurred_at}.${4|day,week,month,quarter,year,fiscal_quarter,fiscal_year|} periods ${5:6}",
  ...
  { label: "fiscal_year_starts", detail: "Open the fiscal year in a month", contexts: ["modelBody"], snippet: "fiscal_year_starts ${1:4}",
    doc: "The month the fiscal year opens, from 1 to 12. Unlocks the `.fiscal_quarter` and `.fiscal_year` grains on this model's time dimensions.\n\n```sem\nmodel Orders {\n  table public.orders\n  primary_key id\n  fiscal_year_starts 4\n}\n```\nCalendar grains are untouched by it — `.year` stays the calendar year, and a fiscal year opening in January shifts nothing at all." },
  { label: "timezone", detail: "Set the model's calendar", contexts: ["modelBody"], snippet: "timezone '${1:Asia/Ho_Chi_Minh}'",
    doc: "The zone whose calendar decides where this model's time grains fall. Without it, `ordered_at.month` breaks on UTC month boundaries — so a sale at 07:00 on the 1st in Ho Chi Minh City lands in the previous month.\n\n```sem\nmodel Orders {\n  table public.orders\n  primary_key id\n  timezone 'Asia/Ho_Chi_Minh'\n}\n```\nThe time column is assumed to hold an absolute instant (`timestamptz` on Postgres, `TIMESTAMP` on BigQuery, UTC on MySQL). Applies to every grain on the model — in `by`, in `where`, and in `retention`." },
  ...
  { label: "hierarchy", detail: "Declare a drill path over dimensions", contexts: ["modelBody"], snippet: "hierarchy ${1:name} = ${2:coarsest} > ${3:finest}",
    doc: "An ordered drill path over dimensions, coarsest first. Pass it to `.of` or `.share` and the subtotal follows the drill: it partitions by the nearest level above whichever level you grouped by.\n\n```sem\nhierarchy geo = country > region > city\n```\n```sem\n# the same query at any depth — always the share of its parent\nshow revenue / revenue.of(geo) as share_of_parent by country, region, city  # of region\nshow revenue / revenue.of(geo) as share_of_parent by country, region        # of country\nshow revenue / revenue.of(geo) as share_of_parent by country                # of the total\n```\nLevels must be dimensions of the same model, and cannot be a time dimension — grains already nest." },
  ...
  { label: "time", detail: "Dimension type", contexts: ["type"], doc: "Dimension type: a timestamp or date. Only a `time` dimension can take a grain — `ordered_at.day`, `.week`, `.month`, `.quarter`, `.year` — which compiles to `DATE_TRUNC`. With `fiscal_year_starts` on the model, `.fiscal_quarter` and `.fiscal_year` are available too, and with `timezone` the grain is cut on that zone's calendar." },
  ...
  { label: "as", detail: "Alias or materialization body", contexts: ["topLevel", "query"], doc: "Names a shown column, or introduces a `materialize` or `rollup` body.\n\n```sem\nshow revenue / revenue.of(region) as share_of_region by region, status\n\nrollup daily_revenue as\n  show revenue by ordered_at.day\n```\nAn inline expression in `show` has no name of its own, so `as` is required there. A plain metric already has one, and `as` renames it." },
