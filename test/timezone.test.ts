...
describe("timezones per dialect", () => {
  test("bigquery switches to timestamp_trunc, the only one of its truncations that takes a zone", () => {
    const { sql } = compile(LOCAL, "show revenue by ordered_at.month", { dialect: bigquery });
    expect(sql).toContain("TIMESTAMP_TRUNC(orders.ordered_at, MONTH, 'Asia/Ho_Chi_Minh')");
  });
